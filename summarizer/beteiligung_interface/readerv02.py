...
class readerv02:
    ...
    def get_all_contributions(self, projid, title=False):
        """
        ...
        """
        contribs = self.get_all_contributions_with_meta(projid)
        returnlist = []
        for c in contribs:
            if title:
    ...
    def get_contribution_with_comments(self, projid, nid, title=False):
        """
        ...
        """
        contrib = self.get_contribution(projid, nid, title)
        comments = self.get_all_comments_of_contrib(projid, nid)
        comstr = ""
        for com in comments:
            comstr = comstr + " " + com
        # comstr has leading " "
    ...
    def get_contributions_count(self, projid):
        """
        Count all contributions in a project
        :param projid: ID of the project
        :return: int, number of contributions
        """
        list = self.get_all_contributions(projid)
        return len(list)